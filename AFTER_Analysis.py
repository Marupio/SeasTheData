...
class AnalysisEngine:
    ...
    def extrapolate_year(self, target_year: int):
        """Fit linear regression to df_yearly_rolling and extrapolate to past or
        future."""
        logging.info(f"Extrapolating to year {target_year}")

        # Restore original if it exists, otherwise back it up
        ...
        oldest_year = df.index[0]
        newest_year = df.index[-1]

        if target_year >= oldest_year and target_year <= newest_year:
            logging.warning(
        ...
        for col in df.columns:
            ...
            if target_year > newest_year:
                ...
                future_years = np.arange(newest_year + 1, target_year + 1).reshape(-1, 1)
                ...
                # Extrapolate into the past - 'retropolating'?
                past_years = np.arange(target_year, oldest_year).reshape(-1, 1)
                y_past = model.predict(past_years)
                projected_df = pd.DataFrame(
                    {col: y_past},
                    index=past_years.flatten()
                )

            projections[col] = projected_df[col]

        df_projected = pd.concat(projections.values(), axis=1)
        df_projected.columns = df.columns
        df_projected.index.name = 'Year'

        # Append to top or bottom depending on direction
        if target_year < oldest_year:
            self.df_yearly_rolling = pd.concat([df_projected, df])
        else:
            self.df_yearly_rolling = pd.concat([df, df_projected])
